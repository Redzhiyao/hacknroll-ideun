...
function CharacterSelect() {
  ...
  // ✅ NEW: pabo (5 frames on select screen)
  const paboName = character("pabo/pabo.svg");
  const paboFrames = [
    character("pabo/pabo normal state/pabo normal A.svg"),
    character("pabo/pabo normal state/pabo normal B.svg"),
    character("pabo/pabo normal state/pabo normal C.svg"),
    character("pabo/pabo normal state/pabo normal D.svg"),
    character("pabo/pabo normal state/pabo normal E.svg"),
  ];
  ...
  const characters = [
    ...
    { id: "pabo-normal-a" as const, nameSvg: paboName, frames: paboFrames },
  ];
  ...
  // ✅ FIX: frame loop uses current.frames.length (so pabo can be 5 frames)
  React.useEffect(() => {
    setFrame(0);

    const fps = 2.5;
    const ms = Math.floor(1000 / fps);
    const framesLen = current.frames.length;

    const t = window.setInterval(() => {
      setFrame((f) => (f + 1) % framesLen);
    }, ms);

    return () => window.clearInterval(t);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [idx]); // rerun when character changes

  const onPrev = () => setIdx((i) => (i - 1 + characters.length) % characters.length);
  const onNext = () => setIdx((i) => (i + 1) % characters.length);

  const onSelect = async () => {
    console.log("🎯 Selecting character:", current.id);
    await setSettings({ characterId: current.id });
    console.log("✅ Settings saved with characterId:", current.id);
